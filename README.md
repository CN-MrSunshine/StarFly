...
### 环境要求
- **操作系统**：Windows 10/11（64 位）其他架构暂不支持
- **编译器**：GCC (w64devkit)
- **依赖头文件**：
  - `windows.h`
  - `stdio.h`
...
### 运行步骤
1. 以管理员权限运行 `StarFlyr.exe`。
2. 程序将自动：
   - 定位并提升对 `winlogon.exe` 的权限。
   - 注入 shellcode。
   - 输出注入过程的日志。
4. 注入完成后，运行任意需要 UAC 提升的程序（例如 `cmd.exe /c dir`）。
5. 检查是否弹出计算器窗口，验证注入成功。
...
- **已知问题**: 本项目尚未实现模拟ImpersonateLoginOnUser的底层调用，目前ImpersonateLoginOnUser直接调用易触发用户模式Hook
